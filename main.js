...
denz.autoReconnect = ReconnectMode.onConnectionLost
    ...
    denz.browserDescription = ['LeonGanz','Desktop','3.0']
    await sleep(10000)
    denz.on('qr', qr => {
        qrcode.generate(qr, { small: true })
        console.log(color('|TRM|'), color('Scan this QR code', 'cyan'))
    })
    fs.existsSync('./Yui.json') && denz.loadAuthInfo('./Yui.json')
...
fetch(`http://ip-api.com/line`).then(res => res.text())  
   ...
   denz.on('chat-update', async (mek) => {
	...
	denz.on('group-update', async (anu) => {
    	...
    	const fkontakk = { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(anu.jid ? { remoteJid: '6283136505591-1604595598@g.us' } : {})}, message: { "contactMessage":{"displayName": `${metdata.subject}`,"vcard":`BEGIN:VCARD\nVERSION:3.0\nN:2;Denz;;;\nFN:Denz\nitem1.TEL;waid=6282386767812:6282386767812\nitem1.X-ABLabel:Mobile\nEND:VCARD` }}}
...
denz.on('CB:action,,call', async json => {
        ...
        denz.sendMessage(callerId, { displayname: `${NamaOwner}`, vcard: vcard}, MessageType.contact, {contextInfo: { externalAdReply:{title: `Developer ${NamaBot}`,body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./denz.jpg'),sourceUrl:`https://wa.me/6282386767812?text=Assalamualaikum`}}})
        await sleep(5000)
